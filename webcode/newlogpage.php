...
<?php
	    ...
	    $simp = "SELECT * FROM monitoring";
		$aidcon = mysqli_query($conn, $simp);
		
		$simp1 = "SELECT DATE_FORMAT(date_time, '%M %d %Y') AS DATE FROM monitoring;";
		$aidcon1 = mysqli_query($conn, $simp1);
		
		$simp2 = "SELECT TIME_FORMAT(date_time, '%H:%i:%s %p') AS TIME FROM monitoring;";
